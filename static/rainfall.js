(function(){
    ...
    Map.prototype = {
        initGrid: function() {
            ...
            var div = d3.select("body").append("div")
                .attr("class", "tooltip")
                .style("opacity", 0);
            d3.json(DATA_DIR + 'grid.json', function(error, json) {
                console.log(error);
                ...
                .on("mouseover", function(d, i) {
                    ...
                    .style("opacity", 1);
                    dataline = map.data[map.currentRow];
                    div.text(dataline[i])
                    //div.text(parseFloat(d2.id))
        ...
        //d3.json(DATA_DIR + "french_guiana.json", function(error, uk) {
        //    var boundaries = topojson.feature(uk, uk.objects.french_guiana);
        d3.json(DATA_DIR + "guiana.json", function(error, uk) {
            var boundaries = topojson.feature(uk, uk.objects.guiana);
        ...
        this.boundaries = map.svg.append("path")
                    .datum(boundaries)
                    .attr("d", map.path)
                    .style('fill', 'none')
            ...
            //    .data(topojson.feature(uk, uk.objects.french_guiana).features)
            //    .enter().append("path")
            //    .attr("class", function(d) { return "subunit " + d.id; })
            //    .attr("d", map.path);

            //map.svg.append("path")
            //    .datum(topojson.mesh(uk, uk.objects.french_guiana, function(a, b) { return a !== b; }))
            //    .attr("d", map.path)
        ...
        makeNavBar: function() {
            ...
            // this.getRowStats();
            nbscale = d3.scale.linear().domain([0.5, this.rowStats.av.length - 0.5]).range([0, NAVBAR_WIDTH]);
        ...
        updateGrid: function(row) {
            ...
            this.currentRow = row;
            console.log('Setting map to date ' + dataline.date);
            $('#subtitle').text(MONTH_NAMES[dataline.date.getMonth()] + ' ' + dataline.date.getDate() + ', ' + dataline.date.getFullYear());
            this.gridpoints
    ...
    var setTitles = function() {
        ...
        //$('#data-sample').text('From ' + MONTH_NAMES[dateLimits[0].getMonth()] + ' ' + dateLimits[0].getDate() + ', ' + dateLimits[0].getFullYear() + ' to ' + MONTH_NAMES[dateLimits[1].getMonth()] + ' ' + dateLimits[1].getDate() + ', ' + dateLimits[1].getFullYear());
    };
    
    maps = [];
    maps.push(new Map(DATA_DIR + 'p_data.csv', '#vizsun', 'Rainfall (mm/day)', 'maps', setTitles)); 
