...
#this_host = 'http://localhost:8080' # URL of the host (can be 'http://...' or '' if local host)
...
@app.route('/trmm/static/<filename>', name='trmm/static')
def server_static(filename):
    """Route to static files."""
    resp = static_file(filename, root='trmm/static')
    resp.set_header('Cache-Control', 'no-cache')
    return resp

@app.route('/trmm/data/<filename>', name='trmm/data')
def server_data(filename):
    """Route to data files."""
    resp = static_file(filename, root='trmm/data')
    resp.set_header('Cache-Control', 'no-cache')
    return resp
